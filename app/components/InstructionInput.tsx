...
const InstructionInput: FC<Props> = ({ focusOnMount = false, ...rest }) => {
  ...
  return (
    <div {...rest}>
      <label>
        <div className="flex items-center gap-2 text-sm uppercase font-bold font-favorit text-white">
          <PencilIcon />
          Test Call Guideline
        </div>
        {instructions && (
          <div className="text-xs text-gray-400 mt-3">{instructions && "* Guideline is user-set"}</div>
        )}
        <textarea
          ref={inputRef}
          className="w-full mt-4 p-4 h-40 border border-gray-700 rounded-lg bg-gray-900 text-white resize-none"
          value={text || ""}
